...
import searchIcon from '../../../assets/images/Icons/searchDarkIcon.svg';
// update
import AllFilesCust from '../../customer/cust-components/AllFilesCust';
...
const MyCollections = () => {
  ...
  const deleteFolders = async () => {
    if (categoriesFocus === 'folderImages') {
      const res = await httpClient.delete(
        `/collection_master/deleteCollection/${imagesFolderArray.collectionId}`
      );
      // getUserIdWiseArts();
      getFolders();
  ...
  const handleKeyDownEdit = async (event) => {
    if (event.keyCode === 13 && editName !== '') {
      const object = {
        collectionId: imagesFolderArray.collectionId,
        title: editName,
      };
      // Enter key
    ...
    if (searchObj.text === '') {
      ...
      try {
        const res = await httpClient.post(
          '/collection_master/CollectionFilter',
          searchObj
        );
        // console.log('elseiff');
        // console.log(object);
    ...
    <div>
      <div className='flex justify-center'>
        <div
          ...
          style={{
            background:
              'linear-gradient(90deg, rgba(220,76,154,0.4) 0%, rgba(149,147,205,0.4) 35%, rgba(132,212,156,0.4) 65%, rgba(204,236,59,0.4) 100%)',
          }}
        >
          <div className='flex justify-center z-[99] bg-[#fff]'>
            {/* border-t-[1px] border-[#efefef] border-b-[1px]' */}
      ...
      <div className='w-[100%] flex justify-center pt-[35px]'>
        <div className='w-[1410px] flex justify-between'>
          ...
          <div className='w-[330px] leading-[1] mt-[17px] flex justify-end gap-[20px]'>
      ...
      <div className='w-full pt-[20px]'>
        <div className='flex gap-[30px] max-w-[1410px] mx-[auto]'>
            ...
            {artTabFocus === true &&
              categories.art.map((obj) => (
                <div>
                  ...
                  {imagesFolderArray?.collectionId ===
                    obj.collectionId && editFolder === true ? (
          ...
          <div className='max-w-[1167px] w-[100%]'>
              ...
              {categoriesFocus === 'product' &&
                categories.product.map((image) => (
                  ...
                  >
                    <div
                      style={{
                        backgroundImage: `url(${image.artProductMaster[0]?.images[0].image})`,
                        backgroundColor:
                          image.artMaster === []
                            ? 'transparent'
                            : '#bbbaba',
            ...
            {categoriesFocus === 'folderImages' &&
            imagesFolderArray.category === 'product' ? (
              <div className='flex flex-wrap gap-[16px]'>
                {categoriesFocus === 'folderImages' &&
                imagesFolderArray?.artProductMaster?.length > 0 ? (
                  imagesFolderArray?.artProductMaster?.map(
                    (image) => (
                      <div className='flex flex-col text-center'>
                        ...
                        <p className='text-[11px] text-primaryBlack font-medium pt-[3px]'>
                          {image.artProductName}
                        </p>
                        <p className='text-[11px] text-primaryGray'>
                          Image ID:{image.artProductUniqueNo}
                        </p>
                        <p className='text-[11px] text-primaryGray'>
                          Uploaded: {datePipeReact(image.date)}
                        </p>
                      </div>
                    )
                  )
                ...
                imagesFolderArray?.artMaster?.length > 0 ? (
                  imagesFolderArray?.artMaster?.map((image) => (
                    <div className='flex flex-col text-center'>
                      ...
                      <p className='text-[11px] text-primaryGray'>
                        Image ID:{image.arProductNo}
                      </p>
                      <p className='text-[11px] text-primaryGray'>
                        Uploaded {datePipeReact(image.submittedDate)}
                      </p>
                    </div>
                  ))
                ) : categoriesFocus === 'folderImages' &&
